...
read -p "Do you want to run 'lab clean' to remove generated files in current dir first? (y/n): " confirm
if [[ "\$confirm" == "y" ]]; then
    if [ -f "\$INSTALL_DIR/lab.sh" ]; then
        bash "\$INSTALL_DIR/lab.sh" clean
    fi
fi
...
echo "Uninstall script generated at $UNINSTALL_SCRIPT"
